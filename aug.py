...
def get_normalize():
    ...
    def process(a, b):
        normalize_a = albu.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        normalize_a = albu.Compose([normalize_a], additional_targets={'target': 'image'})
        normalize_b = albu.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        normalize_b = albu.Compose([normalize_b], additional_targets={'target': 'image'})
        r_a = normalize_a(image=a)
        r_b = normalize_b(image=b)
        return r_a['image'], r_b['image']
